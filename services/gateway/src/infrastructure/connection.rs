...
impl Connection {
    pub fn new(id: ID, socket: WebSocketStream<TcpStream>) -> Self {
        Self {
            id,
            socket,
            heartbeat_interval: interval(Duration::from_secs(20)),
            missed_heartbeats: 0,
            closed: false,
        }
    }
    ...
    async fn handle_message(&mut self, msg: Message) -> Result<(), GatewayError> {
        ...
        match msg {
            ...
            Message::PublicKey { encoded_public_key } => {
                self.handle_public_key(encoded_public_key).await?
            }
    ...
    async fn handle_public_key(&mut self, encoded_public_key: String) -> Result<(), GatewayError> {
        ...
        let shared_secret =
            EphemeralSecret::random_from_rng(&mut OsRng).diffie_hellman(&public_key);

        let mut nonce = vec![0u8; 32];
        OsRng.fill_bytes(&mut nonce);

        let encrypted_nonce: Vec<u8> = nonce
            .iter()
            .zip(shared_secret.as_bytes().iter().cycle())
            .map(|(n, s)| n ^ s)
            .collect();

        self.send_message(Message::Nonce {
            encrypted_nonce: BASE64_STANDARD.encode(encrypted_nonce),
        })
        .await;
        Ok(())
    }
    ...
    fn decode_public_key(
        ...
        let decoded_key = BASE64_STANDARD.decode(encoded_public_key).map_err(|e| {
            GatewayError::PublicKeyDeserializationError(format!(
